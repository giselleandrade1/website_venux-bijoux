          ...
          <div>
            {state.items.map(i => (
              <div key={i.id} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <input type="number" value={i.quantity} onChange={e => update(i.id, Number(e.target.value) || 1)} style={{ width: 80 }} />
                <button onClick={() => remove(i.id)}>Remover</button>
              </div>
            ))}
            <div style={{ marginTop: 16 }}>Subtotal: R$ {subtotal.toFixed(2)}</div>
          </div>
        )}
      </div>
    </MainLayout>
