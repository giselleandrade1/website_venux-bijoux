  ...
  switch (action.type) {
    case "ADD": {
      const found = state.items.find(
        (i) => i.id === action.item.id && i.pendantId === action.item.pendantId
      );
      if (found) {
        return {
          items: state.items.map((i) =>
            i.id === action.item.id && i.pendantId === action.item.pendantId
...
export const CartProvider: React.FC<{ children?: React.ReactNode }> = ({
  ...
  return (
    <CartContext.Provider value={{ state, add, remove, update, clear }}>
      {children}
